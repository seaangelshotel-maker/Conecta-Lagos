...
export const Home: React.FC<HomeProps> = ({ currentUser, onNavigate }) => {
  ...
  const { businesses: swrBusinesses, isLoading: businessesLoading } =
    useBusinesses();
  const { categories: swrCategories, isLoading: categoriesLoading } =
    useAppCategories();
        ...
        if (entries[0].isIntersecting) {
          ...
          setTimeout(() => {
            setVisibleCount((prev) =>
              Math.min(prev + 6, filteredBusinessesForFeed.length),
            );
    ...
    if (uniqueHighlights.length > 1) {
      const interval = setInterval(() => {
        setCurrentHighlightIndex(
          (prev) => (prev + 1) % uniqueHighlights.length,
        );
      }, 5000);
  ...
  const isDataLoading =
    loading || couponsLoading || businessesLoading || categoriesLoading;
        ...
        <div className="flex justify-between items-center px-4 py-3 max-w-7xl mx-auto w-full">
          ...
          >
            ...
            <div className="flex flex-col justify-center">
              <span className="text-[10px] uppercase font-black text-slate-400 leading-tight tracking-wider">
                {t("Entregar ou Explorar em")}
              </span>
            ...
            {gpsLoading && (
              <div className="w-2 h-2 rounded-full border-2 border-red-500 border-t-transparent animate-spin ml-2" />
            )}
          </div>
      ...
      <div className="max-w-7xl mx-auto pt-4 relative z-10 space-y-8">
            ...
            <div className="bg-white border border-emerald-100 p-3.5 rounded-2xl flex items-center justify-between gap-3 shadow-2xs hover:shadow-xs transition-shadow">
              <div className="flex items-center gap-3 flex-1 min-w-0">
                ...
                <div className="min-w-0 flex-1 pr-2">
                  <span className="text-[9px] md:text-[10px] text-emerald-700 font-extrabold uppercase tracking-wider block mb-0.5 line-clamp-1">
                    {t("Minha Carteira de Economia")}
                  </span>
              ...
              <div className="flex flex-col items-end shrink-0 pl-2 border-l border-slate-100 gap-0.5">
                <div className="text-[9px] text-slate-400 font-extrabold uppercase tracking-wider leading-none text-right">
                  {t("Total Economizado")}
                </div>
                <div className="text-base font-black text-emerald-600 font-sans leading-tight">
                  R$ {(currentUser.savedAmount || 0).toFixed(2)}
                </div>
              </div>
            </div>
          </div>
        ...
        <div className="px-4">
          <div className="relative w-full h-40 sm:h-52 md:h-[350px] rounded-2xl overflow-hidden shadow-sm bg-slate-100">
            {uniqueHighlights.length > 0 ? (
              uniqueHighlights.map((h, idx) => (
                ...
                >
                  ...
                  <div className="absolute inset-0 bg-gradient-to-tr from-black/80 via-black/30 to-transparent flex flex-col justify-end p-4 md:p-10">
                    <span className="bg-red-600 text-white text-[10px] font-black uppercase px-2 py-1 rounded w-fit mb-2">
                      Novo
                    </span>
                    <h2 className="text-white text-xl md:text-4xl font-black mb-1 drop-shadow-md leading-tight">
                      {h.title}
                    </h2>
                    <p className="text-slate-200 text-xs md:text-sm font-medium line-clamp-1">
                      {h.description}
                    </p>
                  </div>
                </div>
              ...
              <div
                className="w-full h-full bg-red-500 flex items-center justify-center p-6 text-white cursor-pointer"
                onClick={() => onNavigate("search")}
              >
                <div className="text-left w-full h-full relative">
                  <span className="bg-white text-red-600 text-[10px] font-black uppercase px-2 py-1 rounded-full w-fit mb-2">
                    Desconto
                  </span>
                  <h2 className="text-2xl md:text-4xl font-black mb-1 leading-tight tracking-tight mt-2 w-2/3">
                    Festival de Vantagens
                  </h2>
                  <p className="text-sm font-medium opacity-90 max-w-[60%]">
                    Atrações com preços especiais
                  </p>
                </div>
              </div>
            )}
          ...
          {isDataLoading ? (
            <div className="flex gap-4 overflow-hidden px-4">
              {[1, 2, 3, 4, 5, 6].map((i) => (
                <div
                  key={i}
                  className="w-16 h-20 bg-slate-200 rounded-lg animate-pulse shrink-0"
                ></div>
              ))}
            </div>
          ) : (
            <div className="grid grid-flow-col auto-cols-max gap-3 md:gap-6 overflow-x-auto px-4 pb-2 hide-scrollbar">
              {uniqueCategories.map((cat) => {
                ...
                if (
                  ...
                  emoji = "⛵";
                if (n.includes("serviço") || n.includes("delivery"))
                  emoji = "🛵";
                ...
                return (
                  <div
                    ...
                    onClick={() => onNavigate("guide", { category: cat.name })}
                  >
                    <div className="w-[72px] h-[72px] rounded-2xl bg-white shadow-sm border border-slate-100 flex flex-col items-center justify-center text-red-500 overflow-hidden relative active:scale-95 transition-transform group-hover:bg-slate-50">
                      <div className="text-3xl relative z-10 drop-shadow-sm">
                        {emoji}
                      </div>
                    </div>
                    <span className="text-[11px] font-semibold text-slate-700 text-center leading-tight line-clamp-2 md:text-xs group-hover:text-red-500">
                      {t(cat.name)}
                    </span>
                  </div>
                );
        ...
        {blogPosts.length > 0 && (
          <div className="px-4">
            <div className="flex justify-between items-center mb-4 px-0.5">
              ...
              <button
                className="text-[10px] font-black text-red-500 hover:text-red-600 uppercase tracking-widest transition-transform duration-200 active:scale-95"
            ...
            <div className="flex gap-5 overflow-x-auto hide-scrollbar pb-3 -mx-4 px-4 md:mx-0 md:px-0">
              {blogPosts.slice(0, 4).map((post) => {
                return (
                  <div
                    key={post.id}
                    onClick={() =>
                      onNavigate("blog-detail", { postId: post.id })
                    }
                    className="relative h-72 md:h-80 w-[270px] sm:w-[320px] md:w-full shrink-0 rounded-[1.75rem] overflow-hidden shadow-xs group cursor-pointer active:scale-98 transition-transform border border-slate-100 flex flex-col justify-end p-5"
                  >
                    {/* Cover Image */}
                    <img
                      src={post.imageUrl}
                    ...
                    {/* Top tag */}
                    <span className="absolute top-4 left-4 bg-white/95 text-[9px] font-black text-slate-800 px-2.5 py-1 rounded-lg uppercase tracking-wider shadow-xs z-10">
                      {post.category}
                    </span>
                    ...
                    <div className="relative z-10">
                      ...
                      <h4 className="text-white font-black text-sm sm:text-base leading-snug group-hover:text-red-400 transition-colors line-clamp-2 drop-shadow-sm">
                        {post.title}
                      </h4>
                    </div>
                  </div>
        ...
        {uniqueCollections.length > 0 && (
          <div className="px-4">
            <div className="flex justify-between items-center mb-4 px-0.5">
              ...
              <button
                className="text-[10px] font-black text-red-500 hover:text-red-650 uppercase tracking-widest transition-transform duration-200 active:scale-95"
            ...
            <div className="grid grid-flow-col auto-cols-max overflow-x-auto hide-scrollbar gap-3 pb-2 -mx-4 px-4 md:mx-0 md:px-0">
              {uniqueCollections.map((collection) => {
                ...
                const bgOp =
                  collection.gradientOpacity != null
                    ? collection.gradientOpacity
                    : 0.8;
                ...
                return (
                  ...
                  >
                    ...
                    <div
                      className="absolute inset-0"
                      style={{ backgroundColor: bgColor, opacity: bgOp * 0.3 }}
                    ></div>

                    <span className="text-white font-bold text-sm md:text-lg leading-tight md:max-w-32 z-10 drop-shadow-md">
                      {t(collection.title)}
                    </span>
                  </div>
                );
        ...
        <div>
          ...
          {visibleCount < filteredBusinessesForFeed.length && (
            <div
              ref={homeLoaderRef}
              className="flex flex-col items-center justify-center py-8 text-slate-400 font-semibold text-xs gap-2"
            >
